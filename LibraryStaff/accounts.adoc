...
. They can update the main patron profile permission group and working location of a staff account unless an account has LSA permissions (in which case we will).
. If an account has reporter permissions, we need to check if the account is running any recurring reports. Run the report "Reports Run In Specified Time Period By Specified Library" (in the Recurring? column, 1 = true 0 = false), showing at least the last ~5 weeks.
...
.. If the staff person whose account is being retired has already left, we can determine
... whether the reports can just be cloned from the shared Sitka templates
... whether they need to be cloned at all.
