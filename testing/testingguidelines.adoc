...
we are affected by the bug. Add the Launchpad bug number to the RT ticket and let the library know it's a known issue.
.. If not already in the bug, add a comment to indicate that it affects the version of Evergreen you tested.  If a 
...
. Tes on the upgrade server and compare the behaviour with current documentation and the production server.
