...
Sitka test servers: https://wiki.libraries.coop/doku.php?id=sitka:testing_server_info
...
the results you got.
. Search Launchpad to see if a bug already exists for the reported issue.
.. If you find an existing bug click *This bug affects X people. Does this bug affect you?* to indicate that
we are affected by the bug.  Add a comment link it to the RT ticket and let the library know it's a known issue.
.. If not already in the bug, add a comment to indicate that it affects the version of Evergreen you tested
. If a bug report doesn't exist yet further testing is needed.
...
. If the issue can be replicated on an Evergreen community test server report the issue to Launchpad.
...
. Use testing checklist for the upgrade as a guide. 
  ..  This include a a list of the new and existing features or functionalities that need to be tested.
...
version of Evergreen.
  .. If it can be replicated it's an upstream bug and needs to be reported to Launchpad.
...
Once reported relevant RT tickets should be updated to include the Launchpad bug number.
